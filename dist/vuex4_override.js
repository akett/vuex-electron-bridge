...
class VuexOverride {
  ...
  makeLocalContext(shareCommit, store, namespace, path, isRenderer) {
    ...
    const local = {
      ...
      shareCommit: noNamespace ? store.shareCommit : (_type, _payload, _options) => {
        const args = this.unifyObjectStyle(_type, _payload, _options);
        const {
          payload,
          options
      ...
      localCommit: noNamespace ? store.localCommit : (_type, _payload, _options) => {
        const args = this.unifyObjectStyle(_type, _payload, _options);
        const {
          payload,
          options
    ...
    Object.defineProperties(local, {
      getters: {
        get: noNamespace ? () => store.getters : () => this.makeLocalGetters(store, namespace, options)
      },
      state: {
  ...
  makeLocalGetters(store, namespace) {
    if (!store._makeLocalGettersCache[namespace]) {
      ...
      Object.keys(store.getters).forEach(type => {
        ...
        const localType = type.slice(splitPos); // Add a port to the getters proxy.
